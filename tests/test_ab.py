...
class TestScriptAbWebRoute:
    def _create_project_with_idea(self, client) -> tuple[int, int, str]:
        """Создаёт проект + идею. Возвращает (project_id, idea_id, slug)."""
        name = f"AB-веб-тест {uuid.uuid4().hex[:6]}"
        resp = client.post(
            "/projects/new",
            data={
                "name": name,
        ...
        with get_db() as db:
            ...
            for platform in ("telegram", "vk"):
                db.execute(
                    "INSERT OR REPLACE INTO project_platforms (project_id, platform, enabled, mode) VALUES (?, ?, 1, 'auto')",
                    (project_id, platform),
                )
            cur = db.execute(
                "INSERT INTO ideas (project_id, text, source, status) VALUES (?,?,?,?)",
                (project_id, "AB тестовая идея", "manual", "new"),
            )
            idea_id = cur.lastrowid

        return project_id, idea_id, slug

    def test_htmx_returns_200_with_badge(self, client):
        """HTMX-запрос на /ideas/{id}/script_ab → 200, фрагмент содержит бейдж черновиков."""
        project_id, idea_id, slug = self._create_project_with_idea(client)

        resp = client.post(
            f"/ideas/{idea_id}/script_ab",
            headers={"HX-Request": "true"},
        )
        assert resp.status_code == 200
        # Два черновика создано — бейдж должен показать число
        body = resp.text
        assert "2" in body or "черновик" in body.lower()

    def test_creates_two_content_records_via_web(self, client):
        """POST через веб создаёт ровно 2 контента."""
        project_id, idea_id, slug = self._create_project_with_idea(client)

        resp = client.post(f"/ideas/{idea_id}/script_ab")
    ...
    def test_unknown_idea_returns_404(self, client):
        """Несуществующая идея → 404."""
        resp = client.post("/ideas/99999/script_ab")
        assert resp.status_code == 404

    def test_llm_error_returns_502(self, client, monkeypatch):
        """LLMError → 502 с человекочитаемым сообщением."""
        project_id, idea_id, slug = self._create_project_with_idea(client)

        from app.pipeline import script as script_module

        def mock_generate_script_ab(idea_id):
            raise LLMError("Тестовая ошибка LLM")

        monkeypatch.setattr(script_module, "generate_script_ab", mock_generate_script_ab)

        # Нужно также патчить импортированный generate_script_ab в web/generate.py
        import app.web.generate as web_generate_module
        monkeypatch.setattr(web_generate_module, "generate_script_ab", mock_generate_script_ab)
