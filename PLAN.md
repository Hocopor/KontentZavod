...
- [ ] `[оркестратор]` Спроектировать финально перед волной (см. контракт).
- [ ] `[S]` ВОЛНА A — генерация: catalog, промпт, _generate_story (caption→preview), тесты генерации.
- [ ] `[S]` ВОЛНА B — публикация: vk.py story-ветка, base/scheduling маршрутизация, тесты публикации.
...
- [ ] `[S]` **Картинка гарантирована.** В `_generate_text` картинка уже качается (`media/{id}/post.jpg` через `fetch_image`); убедиться, что путь пишется в `files.image_path` и пост без картинки — явный warning-бейдж в UI (не молчком).
- [ ] `[S]` **Отображение в управлении.** `app/templates/queue/index.html` (борд/карточки) и `app/templates/queue/_modal.html`: для `type='post'/'article'` показывать превью картинки через `GET /files/images/{content_id}.jpg` (роут уже есть, 7-волна D); миниатюра в карточке борда + полноразмер в модалке. Story-слайды уже показываются (8.5B) — выровнять верстку. Аналогично — в ручной очереди (`_get_manual_pending`/шаблон) картинка поста.
- [ ] `[S]` Тесты: `tests/test_queue.py`/`test_image_in_post.py` — модалка поста содержит `<img src="/files/images/{id}.jpg">`; карточка борда — миниатюру.
...
### 9.3 (P1) Перенос даты/времени публикации без отмены/перегенерации
Жалоба (НОВОЕ): во вкладке Публикация нельзя изменить дату/время без отмены или перегенерации.
- [ ] `[S]` Новый роут `POST /queue/items/{content_id}/reschedule` (web/queue.py): принимает `date` + `time` (Form), `UPDATE schedule SET planned_at=? WHERE content_id=? AND status IN ('planned','manual_pending')`; published/manual_done → 422 («уже опубликовано»). Возврат — HTMX-фрагмент модалки/строки борда (морф, без перезагрузки — паттерн 8.8). planned_at — локальное время (нюанс 7.5).
- [ ] `[S]` UI: в `queue/_modal.html` — мини-форма «📅 Перенести» (date+time инпуты, заполнены текущим planned_at, кнопка «Сохранить», `hx-post`, `hx-swap=morph`). Тесты `tests/test_queue.py`: reschedule planned → 200 + planned_at обновлён; published → 422.
