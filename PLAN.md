...
### 9.8 (баг-фикс) Пустой предпросмотр в шахматках «Контент-план» и «Публикация» ✅ (2026-06-13, pytest 122/122 по затронутым наборам)
**Симптом (жалоба пользователя):** клик по карточке/точке в шахматке открывает пустое модальное окно (только крестик), контент не подгружается.
**Корень:** регрессия коммита `2efe41a "убрал скачки"` (этап 8.8) — он добавил morph-поллинг (`hx-trigger="every 15s" hx-swap="morph:outerHTML"`) на зоны `#plan-board-area` / `#queue-live`, внутри которых лежат карточки. Открытие модалки держалось на ДВУХ независимых вещах при клике: inline `onclick="openModal(event)"` (открывает оверлей) и htmx-атрибуты `hx-get`/`hx-target` (грузят контент). После перморфа зоны htmx-биндинг на переморфленных карточках перестаёт слать GET, а inline onclick (обычный DOM-атрибут, idiomorph его сохраняет) продолжает открывать ПУСТОЙ оверлей. Сервер-эндпоинты исправны (200 + контент, тесты зелёные) — баг чисто клиентский.
**Фикс:** загрузка контента перенесена в `openModal(evt, el)` через `htmx.ajax('GET', el.dataset.modalUrl, {target:'#modal-content', swap:'innerHTML'})` — привязана к надёжному inline onclick, который перморф не ломает; добавлена заглушка «Загрузка…», чтобы окно никогда не было пустым. На карточках/точках `hx-get/hx-target/hx-swap` заменены на `data-modal-url` + `onclick="openModal(event, this)"`. Удалён мёртвый `#modal-trigger-container` (ссылался на несуществующую `openModalAfterLoad()`).
- [x] `[оркестратор]` Диагностика (сервер/DOM/тесты проверены, причина — morph-поллинг + двойной механизм открытия).
- [x] `[S-haiku]` Правки в `app/templates/plan/board.html` и `app/templates/queue/index.html` (5 точечных замен). Серверный код и тесты не трогались; 122/122 затронутых тестов зелёные. НЕ закоммичено.
...
- 2026-06-13 (9.8): **Пустой предпросмотр в шахматках — клиентский баг от morph-поллинга.** Открытие модалки нельзя держать на htmx-атрибутах элемента, который живёт внутри morph-перморфленной зоны: после перморфа htmx-биндинг на ноде перестаёт срабатывать, а сохранённый inline `onclick` открывает пустой оверлей. Правило на будущее: внутри live-зон (`hx-swap="morph:*"`) загрузку по клику делать через `htmx.ajax(...)` из inline-обработчика (он переживает перморф), а не через `hx-get`/`hx-target` на самой ноде.
