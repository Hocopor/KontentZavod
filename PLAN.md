...
- [ ] `[O+S]` Реализация по волнам A→E (см. контракт).
...
- [ ] **C `[O]`** Слоты и планнер v2: slots.py (+легаси-слой), planner.py понедельный цикл со slot_id-заполнением, plan.txt v2, _FAKE_PLAN v2, brain.py без изменений интерфейса, plan_items.goal заполняется, тесты (главный: mix 7/нед → ровно 7 слотов/нед).
- [ ] **D `[S]`** UI директив: блок «Директивы» на странице стратегии (список, статус, «отклонить»), check_warnings жёлтым, форма revise поясняет «комментарий станет директивами», goal-бейдж в модалке плана, тесты.
- [ ] **E `[S]`** E2e: сценарий «пользователь сказал 7 постов/нед» → директива → слоты → ровно 7/нед в плане; revise меняет фазу; легаси-стратегия продолжает работать. DEPLOY.md: миграции.

### 8.3 (P0) Безопасность контента — закон РФ и 18+
Инциденты: «кастрация» в тексте, мужская пара как романтическая сцена в видео. Риск — статья, для РФ-рынка недопустимо.
- [ ] `[S]` Слой «цензор»: базовые запреты в AGENTS.md/промптах ВСЕХ генераций (18+, ЛГБТ-пропаганда по закону РФ, наркотики, экстремизм, оскорбление чувств, мат, политика — список в отдельном файле `app/prompts/agents/forbidden_ru.md`) + пост-проверка сгенерированного отдельным дешёвым LLM-вызовом (purpose='censor': verdict + причина; fail → регенерация с указанием причины, ≤2 попыток, потом status=error с пояснением).
- [ ] `[S]` Цензор применяется к: текстам постов, сценариям видео (текст + keywords сцен ДО поиска футажей), оверлеям сторис, caption'ам.
