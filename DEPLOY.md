...
### Миграции БД (этап 8.2 — маркетинговый мозг)

Дополнительных действий **не требуется** — миграции идемпотентны и применяются при рестарте:

- **Миграция 6:** колонка `plan_items.goal TEXT` (NULL допустим) — маркетинговая цель пункта
  (`attract|retain|sell|brand`), проставляется планнером из фазы стратегии. Добавляется через
  `ALTER TABLE ... ADD COLUMN` под `try/except` (повторный старт безопасен).
- **Миграция 7:** новая таблица `directives` (`CREATE TABLE IF NOT EXISTS`) — структурированные
  директивы пользователя (из `user_comment` при пересмотре стратегии). Количественные директивы
  (`per_week`) применяются КОДОМ в `slots.py` как override микса фазы — указание «по N в неделю»
  физически не может быть проигнорировано LLM.

Стратегии старого формата (плоский `content_mix`, строковые рубрики) продолжают работать через
слой совместимости в `slots.py` — БД не мигрируется, ничего вручную править не нужно.
